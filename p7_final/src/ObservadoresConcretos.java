...
/**
 * @brief ObservadorBolsaValores class. (Observador concreto - Concrete observer).
 * @details Clase que implementa la interfaz Observador, y que define el comportamiento del observador concreto
 * que recibe notificaciones sobre los datos financieros de la Bolsa de Valores obtenidos de la API de Profit.
 */
class ObservadorBolsaValores implements Observador {
    private final String nombre;                                            /**< Nombre del observador de la Bolsa de Valores. */
    private final ArrayList<Valor> valoresAsociados = new ArrayList<>();    /**< Lista de activos de la Bolsa de Valores. */
    private ObservadorWindow window;                                        /**< Ventana que muestra los detalles de los activos de la Bolsa de Valores. */

    /**
     * @brief Constructor.
     * @param nombre [String] Nombre del observador de la Bolsa de Valores.
     */
    public ObservadorBolsaValores(String nombre) {
        this.nombre = nombre;
    }

    /**
     * @brief Metodo que se invoca cuando el sujeto (FinalcialData) notifica un cambio en los datos de la Bolsa de Valores.
     * @param data [String] Datos a actualizar.
     */
    @Override
    public void actualizar(String data) {
        System.out.println("Observador " + nombre + " actualizó sus datos de Bolsa de Valores.");
        ...
        for (JsonElement element : jsonArray) {
            ...
            String name = objeto.get("name").getAsString();
            double lastPrice = objeto.get("price").getAsDouble();
            double changePercentage = objeto.get("change_1d_prc").getAsDouble();

            valoresAsociados.add(new Valor("Bolsa de Valores", name, lastPrice, changePercentage));
        }

        // Mostrar los datos en una ventana gráfica.
        if (window == null) {
            window = new ObservadorWindow("Bolsa de Valores", this.nombre, valoresAsociados);
            window.setVisible(true);
        } else {
            window.actualizarDatos(valoresAsociados);
        }
